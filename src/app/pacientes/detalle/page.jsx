...
function DetallePacienteContent() {
  ...
  const generarToken = async () => {
    ...
    try {
      ...
      const data = await completarDatosService.generarTokenDesdePaciente(parseInt(idPaciente))
      setTokenGenerado(data)
      
      // Si el paciente tiene WhatsApp, mostrar directamente la opción de WhatsApp
      if (paciente?.celular_whatsapp && data?.link_completar_datos) {
  ...
  const compartirPorWhatsApp = async (linkCompletarDatos = null) => {
    ...
    let link = linkCompletarDatos || tokenGenerado?.link_completar_datos
    ...
    try {
      // Si no se pasó el link como parámetro, usar la función para obtener el link correcto
      if (!linkCompletarDatos) {
        link = obtenerLinkCorrecto()
      }
      const urlWhatsApp = generarUrlWhatsAppCompletarDatos(paciente, link)
